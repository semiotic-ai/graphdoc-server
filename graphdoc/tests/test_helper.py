...
class TestHelper:
    ...
    def test_check_directory_path(self):
        ...
        assert check_directory_path(schema_directory_path) is None

    def test_check_file_path(self):
        with pytest.raises(ValueError):
            check_file_path("invalid_path")

        schema_file_path = BASE_DIR / "graphdoc" / "tests" / "assets" / "schemas" / "opensea_original_schema.graphql"
        assert check_file_path(schema_file_path) is None