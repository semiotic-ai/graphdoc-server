...
try:
    ...
    model_info = mlflow.dspy.log_model(
        ...
        metadata={
            ...
            "trainset_version": str(datasets.Version("1.0.0")),
    ...
    mlflow.log_params(
        {
            "trainset": "semiotic/graphdoc_schemas",
            "trainset_version": str(datasets.Version("1.0.0")),
            "model_name": DSPY_MODEL_NAME,
        }
    )
    mlflow.log_metrics(
        {
            "optimized_model_score": str(round(pred_score / len(test_split), 2)),
            "original_model_score": str(round(og_score / len(test_split), 2)),
        }
    )
