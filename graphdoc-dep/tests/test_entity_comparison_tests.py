...
class TestEntityComparison:
    ...
    async def test_ecpe_entity_comparison_revision_prompt(
        self, ecpe, entity_comparison_assets, request
    ):
        """
        ...
        test_cache_file_path = (
            CACHE_DIR / "test_ecpe_entity_comparison_revision_prompt.pkl"
        )
        ...
        if os.path.exists(CACHE_DIR) and fire:
            ...
            tasks = [
                asyncio.to_thread(
                    ...
                    template_name="entity_comparison_prompt.txt",
                    template_variables={
                        "entity_pred": entity_comparison_assets[
                            "gold_entity_comparison"
                        ],
        ...
        for test_asset_comparison in parsed_test_asset_comparisons:
            assert test_asset_comparison["correctness"] in [
                1,
                2,
                3,
                4,
            ], f"Unexpected correctness: {test_asset_comparison['correctness']}"
            assert isinstance(
